...
# ratio_dim = pd.read_sql('select * from ratio_dim ',engine)
# company_dat1 = pd.read_sql('select * from company_dim ',engine)
# for i in range(2008,2018):
#     data = pd.read_sql('select * from ratio_fact where calendar_year = 2018'.format(str(i)),engine)
#     data_m1 = data.merge(ratio_dim,on ='ratio_seq_id',how = 'inner')
#     data_m2 = company_dat1.merge(data_m1,on ='company_seq_id',how = 'inner')
#     data_m2[['company_seq_id', 'company_name','company_primary_symbol','company_category_code', 'company_category', 'company_sector', 'last_fiscal_year_end','date_seq_id','ratio_seq_id', 'financial_period', 'calendar_quarter', 'calendar_year','ratio_value', 'ratio_category', 'ratio_item', 'ratio_name','ratio_formulae', 'N', 'M', 'U', 'T', 'B', 'I', 'format_field','ratio_display_name']].to_csv(r'E:\Learning work\ratio_data_{}.csv'.format(str(i)))
# # logger.info("Ratio Calulation for delta model Started")
#changing the d
...
for i in range(100000):
    ...
    if b !=1 and sum(fib_value) < max_value :
        ...
        a,b = (b + (a + b)),c2
        fib_value.append(c2)
        if sum(fib_value) > max_value:
            print(sum(fib_value[:-1]))
            print(fib_value[:-1])
            print('break')
    else:
        if sum(fib_value) < max_value:
            c1 = b + (a + b)
            # sum_value = sum_value + c1
            # sum_value = sum_value if sum_value < max_value else sum_value - c1
            a,b = (a + b),c1
            fib_value.append(c1)


mul = [i for i in range(1,1000) if i % 3 == 0 or i % 5 == 0]
